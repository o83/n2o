...
impl Selector {
    pub fn new() -> io::Result<Selector> {
        ...
        let kq = try!(kqueue().map_err(from_nix_error));
    ...
    pub fn register(&self,
                    ...
                    opts: PollOpt)
                    -> io::Result<()> {
        println!("registering; token={:?}; interests={:?}", token, interests);
    ...
    fn flush_changes(&self) -> io::Result<()> {
        let result = kevent(self.kq, self.changes(), &mut [], 0)
            .map(|_| ())
            .map_err(from_nix_error)
            .map(|_| ());
