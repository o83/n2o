...
impl<S: AsyncPipeStub> Active<S> {
    ...
    fn on_recv_progress(&mut self,
                        ctx: &mut Context,
                        progress: Result<Option<Message>>)
                        -> Result<()> {
        progress.map(|recv| if let Some(msg) = recv {
            self.on_msg_received(ctx, msg)
        })
    }
    ...
    fn hang_up_changed(&mut self, hup: bool) -> Result<()> {
        if hup {
            self.writable = false;
            self.readable = false;
            Err(Error::new(ErrorKind::Other, "hup"))
        } else {
            Ok(())
        }
    }
...
impl<S: AsyncPipeStub + 'static> PipeState<S> for Active<S> {
    ...
    fn ready(mut self: Box<Self>, ctx: &mut Context, events: Ready) -> Box<PipeState<S>> {
        let res = self.readable_changed(ctx, events).and_then(|_| {
            self.writable_changed(ctx, events).and_then(|_| self.hang_up_changed(events.is_hup()))
        });
