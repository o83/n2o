...
macro_rules! log {
    ($event:expr) => {
        if ::iterators::runner::log::DUMP_LOGS { println!("{:?}", $event); }
    }
}
...
macro_rules! stat_stolen {
    () => {
        ::iterators::runner::log::STOLEN_JOB.fetch_add(1, ::std::sync::atomic::Ordering::SeqCst);
    }
}
...
macro_rules! stat_popped {
    () => {
        ::iterators::runner::log::POPPED_JOB.fetch_add(1, ::std::sync::atomic::Ordering::SeqCst);
    }
}
...
macro_rules! dump_stats {
    () => {
        {
            let stolen = ::iterators::runner::log::STOLEN_JOB.load(::std::sync::atomic::Ordering::SeqCst);
            println!("Jobs stolen: {:?}", stolen);
            let popped = ::iterators::runner::log::POPPED_JOB.load(::std::sync::atomic::Ordering::SeqCst);
