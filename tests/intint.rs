...
#[target_feature = "+avx"]
pub fn mul_array<'a>(x: &mut [i64], y: &[i64]) {
    let n: Vec<i64> = x.iter().zip(y).map(|(d,c)|d*c).collect();
    println!("{:?}",n);
}

fn main() {
    let mut fa = vec![0.0, 1.0, 4.0, 9.0, 0.0, 1.0, 4.0, 9.0];
    let fb     = vec![0.0, 2.0, 6.0, 0.0, 0.0, 1.0, 4.0, 9.0];
