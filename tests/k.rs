...
#[test]
pub fn k_symbols() {
    assert_eq!(ast::parse("`a`b`c;`1`1`1"),
...
#[test]
pub fn k_assign() {
    assert_eq!(ast::parse("a:b:c:1"),
               AST::Assign(AST::Name(String::from("a")).boxed(),
...
#[test]
pub fn k_func() {
    assert_eq!(format!("{:?}", ast::parse("{x*2}[(1;2;3)]")),
               "Call(Lambda(Name(\"x\"), Verb(Times, Name(\"x\"), Number(2))), \
...
pub fn k_adverb() {
    assert_eq!(format!("{:?}", ast::parse("{x+2}/(1;2;3)")),
               "Adverb(Over, Lambda(Name(\"x\"), Verb(Plus, Name(\"x\"), Number(2))), \
                List(Cons(Number(1), Cons(Number(2), Number(3)))))");
}
...
#[test]
pub fn k_reduce() {
    assert_eq!(format!("{:?}", ast::parse("+/{x*y}[(1;3;4;5;6);(2;6;2;1;3)]")),
               "Adverb(Over, Verb(Plus, Nil, Nil), Call(Lambda(Name(\"x\"), Verb(Times, \
...
pub fn k_repl() {
    let mut i = Interpreter::new().unwrap();
    assert_eq!(format!("{}",
                       i.run(ast::parse("y:3;add:{[x]y};f:{[y]add y};f 1"))
                           .unwrap()),
               "3");
}
...
pub fn k_factorial() {
    let mut i = Interpreter::new().unwrap();
    assert_eq!(format!("{}",
                       i.run(ast::parse("fac:{$[x=0;1;x*fac[x-1]]};fac 20").unwrap())),
               "2432902008176640000");
}
...
pub fn k_cond() {
    let mut i = Interpreter::new().unwrap();
    assert_eq!(format!("{}",
                       i.run(ast::parse("a:{[x;y]$[x y;20;10]};a[{x};10]").unwrap())),
               "20");
}
...
pub fn k_cond2() {
    let mut i = Interpreter::new().unwrap();
    assert_eq!(format!("{}",
                       i.run(ast::parse("a:{[x;y]$[x y;20;10]};a[{x};0]").unwrap())),
               "10");
